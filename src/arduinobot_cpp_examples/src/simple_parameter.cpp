...
class SimpleParamater : public rclcpp::Node{
public:
    SimpleParamater() : Node("simple_parameter"){
        declare_parameter<int>("simple_int_param", 28);
        declare_parameter<std::string>("simple_string_param", "Mathias");

        param_callback_handle_ = add_on_set_parameters_callback(std::bind(&SimpleParamater::paramChangeCallback, this, _1));
    }
...
int main(int argc, char* argv[]){
    ...
    auto node = std::make_shared<SimpleParamater>();
